...
int main()
{
	...
	{
		//Загрузка dll
		//Если тут сменить папку dll c Debug на Release, скорость работы увеличиться (перед этим надо собрать release dll)
	...
	catch (const std::exception) //в случае ошибок чистим за собой память
	{
		if (image_io)
			image_io->Release();
