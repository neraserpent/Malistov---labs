...
Container* ProcLibProcess::NormalizeHistogram(IContainer* container)
{
	...
	for (int x = 0; x < container->GetWidth(); x++)
	{
		for (int y = 0; y < container->GetHeight(); y++)
		{
			...
			{
				int color = container->GetRGBPoint(x, y, cur_color); //BYTE* color = reinterpret_cast<BYTE*>(pixel) + i;
				...
				result->SetRGBPoint(x, y, cur_color, val);
			}
		}
	}
...
Container* ProcLibProcess::NormalizeHistogram(IContainer* container)
{
	...
	for (int x = 0; x < container->GetWidth(); x++)
	{
		for (int y = 0; y < container->GetHeight(); y++)
		{
			for (int c = 0; c < 3; c++)
			{
				result->SetRGBPoint(x, y, c, CalculateVal(ab_values, container->GetRGBPoint(x, y, c), c));
			}
		}
	}
...
float ProcLibProcess::CalculateVal(AlphaBeta limits, int colorValue, int limitsIter)
{
	float val;

	val = (colorValue - float(limits.alpha[limitsIter])) / (float(limits.beta[limitsIter]) - float(limits.alpha[limitsIter]));
	val = val < 0.f ? 0.f : val > 1.f ? 255.f : val * 255.f;
